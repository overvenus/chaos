...
func Verify(ctx context.Context, historyFile string, modelNames string) {
	...
	go func() {
		for _, suit := range suits {
			log.Printf("begin to check %s with %s", suit.model, suit.checker)
			...
			if !ok {
				log.Fatalf("%s: history %s is not linearizable", suit.model, historyFile)
			} else {
				log.Printf("%s: history %s is linearizable", suit.model, historyFile)
			}
		}
